...
class ViTExtractor:
    ...
    def create_model(model_type: str) -> nn.Module:
        ...
        # model_type = 'dino_xcit_medium_24_p8'
        print(model_type)
    ...
    def _extract_features(self, batch: torch.Tensor, layers: List[int] = 11, facet: str = 'key') -> List[torch.Tensor]:
        ...
        _ = self.model(batch)
        # print(self._feats[0].shape)
        self._unregister_hooks()
        self.load_size = (H, W)
        self.num_patches = (1 + (H - self.p) // self.stride[0], 1 + (W - self.p) // self.stride[1])
        # print(self._feats[0].shape)#1 6 17877 64
        # print(self.num_patches, len(self._feats[0]))#109 164 | 1
        # exit()
        return self._feats
