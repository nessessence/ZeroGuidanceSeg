...
def _hc_cut(n_clusters, children, n_leaves):
    ...
    if n_clusters > n_leaves:
        raise ValueError('Cannot extract more clusters than samples: '
                         ...
                         % (n_clusters, n_leaves))
    # print(n_clusters, children.shape, n_leaves) #num_parts |  num_pix=1, 2  |  num_pix
    ...
    # print(children[-1])
    print(nodes)
    print(n_clusters, n_leaves) #50 1148
    # exit()
    output = np.zeros((n_clusters*2-2, n_leaves), dtype=np.intp)
    for a in range(n_clusters - 1):
        # print(a, '/', n_clusters - 1)
        # As we have a heap, nodes[0] is the smallest element
        these_children = children[-nodes[0] - n_leaves]
        # Insert the 2 children and remove the largest node
        heappush(nodes, -these_children[0])
        heappushpop(nodes, -these_children[1])
        # print(a, nodes, -these_children[0],-these_children[1])
        
        label = np.zeros(n_leaves, dtype=np.intp)
        for i, node in enumerate([-these_children[0], -these_children[1]]):
            label[_hierarchical._hc_get_descendent(-node, children, n_leaves)] = i+1
        label_1 = (label==1).astype(int)
        label_2 = (label==2).astype(int)
        # label_0 = (label==0).astype(int)
        # print(label_0.sum())
        output[a*2] = label_1
        output[a*2+1] = label_2
        # print(a*2, a*2+1)
    o = output.sum(axis=0)
    # print(np.max(o), np.max(output))
    # exit()
    return output
