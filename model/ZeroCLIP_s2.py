...
import torch
# import clip
# import clip_som2 as clip
import clip_ramp as clip
...
class CLIPTextGenerator:
    ...
    # added
    def get_img_feature_all(self, img_path, weights, mask_bi=None):
        # imgs = [Image.open(x) for x in img_path]
        clip_imgs = [self.clip_preprocess(x).unsqueeze(0).to(self.device) for x in img_path]

        with torch.no_grad():
            image_fts = [self.clip.encode_image(x,mask_bi) for x in clip_imgs]
