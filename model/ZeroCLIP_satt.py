...
import torch
# import clip
import clip_save_selfattn as clip
...
class CLIPTextGenerator:
    ...
    def get_img_feature_all(self, img_path, weights, mask_bi=None):
        ...
        with torch.no_grad():
            image_fts, self_attn = self.clip.encode_image(clip_imgs[0],mask_bi) #for x in clip_imgs
            # print(image_fts)
            # exit()
            ...
            # image_features = image_features / image_features.norm(dim=-1, keepdim=True)
            return image_features.detach(), self_attn
