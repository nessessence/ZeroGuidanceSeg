...
import torch
# import clip
# import clip_v3 as clip
import clip_s as clip
# import clip_tmp as clip
...
class CLIPTextGenerator:
    def __init__(self,
        ...
        if lm_model == 'gpt-neo':
            ...
            # self.lm_tokenizer = GPT2Tokenizer.from_pretrained('distilgpt2')
            # self.lm_model = GPT2LMHeadModel.from_pretrained('distilgpt2', output_hidden_states=True)


            # self.lm_tokenizer = GPT2Tokenizer.from_pretrained('gpt2-xl')
            # self.lm_model = GPT2LMHeadModel.from_pretrained('gpt2-xl', output_hidden_states=True)
        ...
        self.clip, self.clip_preprocess = clip.load("ViT-L/14@336px", device=self.device,
                                                    download_root=clip_checkpoints, jit=False)
        # self.clip, self.clip_preprocess = clip.load("RN50x4", device=self.device, jit=False)


        self.pos_parser = spacy.load("en_core_web_trf")

        # TODO set mask layer
        self.clip.visual.set_mask_layer(mask_layer_list)
    ...
    def get_img_feature2(self, clip_imgs, weights):
        # imgs = [Image.open(x) for x in img_path]
        # clip_imgs = [self.clip_preprocess(x).unsqueeze(0).to(self.device) for x in imgs]

        with torch.no_grad():
            image_fts = [self.clip.encode_image(x) for x in clip_imgs]
            # print( '** ',image_fts[0].shape)
            # exit()
    ...
    # added
    def get_img_feature_all(self, img_path, weights, mask_bi=None, sd_global_subtraction=None,img_name='n'):
        # imgs = [Image.open(x) for x in img_path]
        clip_imgs = [self.clip_preprocess(x).unsqueeze(0).to(self.device) for x in img_path]

        with torch.no_grad():
            image_fts = [self.clip.encode_image(x,mask_bi,sd_global_subtraction,img_name) for x in clip_imgs]
            sal_map = image_fts[0][1]
            image_fts = [image_fts[0][0]]
            if weights is not None:
                image_features = sum([x * weights[i] for i, x in enumerate(image_fts)])
            else:
                image_features = sum(image_fts)
            ...
            # image_features = image_features / image_features.norm(dim=-1, keepdim=True)
            return image_features[0].detach(), None#sal_map.detach()
    ...
    def run(self, image_features, cond_text, beam_size):
        ...
        # context_prefix = ''; cond_text = 'Image of a'


        context_tokens = self.lm_tokenizer.encode(self.context_prefix + cond_text)

        output_tokens, output_text = self.generate_text(context_tokens, beam_size,cond_text)

        return output_text

    def generate_text(self, context_tokens, beam_size,cond_text):
        ...
        for i in range(self.target_seq_length):
            ...
            # # for t,i in vocab.items():
                # parsed_t = self.pos_parser(cond_text +t)
                # print(parsed_t[-1].text, parsed_t[-1].pos_ )
                # print(self.lm_tokenizer.decode(torch.tensor(i)))
                # parsed_t = self.pos_parser(self.lm_tokenizer.decode(torch.tensor(i)).strip() )
                # vocab_pos[t] = parsed_t[0].pos_
                # parsed_t = self.pos_parser(cond_text +' '+  self.lm_tokenizer.decode(torch.tensor(i)) )


            #     print(parsed_t[0].text, parsed_t[0].pos_ )
            # torch.save(vocab_pos,'clip_checkpoints/vocab_pos.pt')
            # logits = filter_pos(logits,context_tokens,gen_tokens)

                # if parsed_t[-1].pos_ not in ['NOUN', 'PROPN']: logits[next_tokens] =  -float(np.inf)

            # try_size = 100
            # _, next_tokens = logits.topk(try_size, -1)
            # next_tokens = next_tokens.permute(1, 0)
            
            # if gen_tokens is None:
            #     try_gen_tokens = next_tokens
            # else:
            #     try_gen_tokens = gen_tokens.expand(try_size, *gen_tokens.shape[1:])
            #     try_gen_tokens = torch.cat((try_gen_tokens, next_tokens), dim=1)


            # output_list = try_gen_tokens.cpu().numpy()
            # output_texts = [
            #     self.lm_tokenizer.decode(output[: int(length)])
            #     for output, length in zip(output_list, seq_lengths)
            # ]

            # output_texts = [ self.lm_tokenizer.decode(output[: 1]) for output in output_list  ]


            # print(try_gen_tokens)
            # print(output_texts)
            # print('yo')


            if scores is None:
                # tokens here refers to token_id
            ...
            tmp_output_texts = [f'{tmp_output_texts[i]} : {tmp_scores[i].item()} ' for i in tmp_order]
    ...
    def get_next_probs(self, i, context_tokens):
        # shape when start with  "Image of a"
        # context_tokens  [4,1] 
        # context_tokens[:, -1:]  [1,1]
        # next   [5,5], [5.1]
        #        [5, 6],[5,1]
        ...
        # optimized context throught the post-hoc optimization
        if context:
